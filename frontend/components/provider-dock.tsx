...
export function ProviderDock({ forceShow, onForceShowChange }: ProviderDockProps) {
  ...
  const handleLoadModels = useCallback(async () => {
    ...
    try {
      const fetched = await listModels(
        selectedProvider,
        provider.needsKey ? apiKey : undefined,
        selectedProvider === "ollama" ? "http://localhost:11434" : undefined,
      );
      setModels(fetched);
      if (fetched.length > 0 && settings?.model) {
        const rawModel = settings.model.replace(/^(openai|anthropic|gemini|ollama)\//, "");
        if (fetched.includes(rawModel)) setSelectedModel(rawModel);
      }
    } catch (e) {
  ...
  }, [selectedProvider, apiKey, settings]);
        ...
        {showPanel && selectedProvider && (
          ...
          >
            ...
            {providerObj?.needsKey && (
              <div className="mb-3">
                ...
                <input
                  ...
                  placeholder={settings?.provider === selectedProvider ? settings.api_key_masked : "Enter your API key"}
                  className="w-full px-3 py-2 text-sm rounded-xl border border-input bg-background outline-none focus:ring-2 focus:ring-ring/30 transition-shadow"
                />
              </div>
            )}
              ...
              {models.length > 0 ? (
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none px-3 py-2 text-sm rounded-xl border border-input bg-background outline-none focus:ring-2 focus:ring-ring/30 transition-shadow pr-8"
                  >
            ...
            >
              ...
              {saving ? "Saving..." : selectedModel ? "Save" : "Select a model"}
            </Button>
          </motion.div>
        )}
      </AnimatePresence>
